...
Weave scripts (`weave.sh`) are **fully autonomous experiments** — they create their
...
continues even if you close the tab or disconnect. Output is captured to the
log file specified in `weave.json` (`log_file` field, default: `weave.log`).

Each weave has a `weave.json` config:
```json
{"run_script": "weave.sh", "log_file": "weave.log"}
```
- `run_script`: which script the Run button executes (default: `weave.sh`)
- `log_file`: where stdout/stderr go — viewable in a console tab via "View Log"
- `weave.json` is the required marker file for a weave
- While running, `weave.json` gets an `active_run` object with `run_id`, `pid`, `pgid`, `started_at`, `script`, and `args` (actual values including `SLICE_NAME`). Cleared on completion.

A `weave.sh` may create one slice, multiple slices in sequence, or parallel slices
...
The WebUI Run modal dynamically renders input fields from these definitions. Each arg
...
```
- **Weave** (`weave.json`): Full topology + run script + args (all in `weave.json`)
