...
1. **List all sites**: `query_sites` — every site with cores/RAM/disk availability,
   state, and special components (GPUs, FPGAs, SmartNICs, NVMe).

2. **Site details**: `query_sites(site_name="STAR")` — detailed info
   including per-host breakdown.

3. **Per-host resources**: `get_site_hosts(site_name="STAR")` — individual host machines.

4. **Find by hardware**: `query_sites(component="GPU_A40")` — sites with
   specific components and minimum resources.

5. **Available images**: `list_images` — all VM images with default users.

6. **Component catalog**: `list_component_models` — all NIC, GPU, FPGA, NVMe models.

## Recommendations

Based on the user's needs:
- **Most capacity**: Sort `query_sites` by `cores_available` descending.
- **GPUs**: `query_sites(component="GPU_A40")` (or GPU_RTX6000, GPU_TeslaT4, GPU_A30).
- **FPGAs**: `query_sites(component="FPGA_Xilinx_U280")`.
...
- Some sites may be in maintenance — `query_sites` shows site state.
