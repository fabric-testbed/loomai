...
1. **Check current lease**:
   - `get_slice(slice_name)` — look at `lease_end` field
   - If no slice name given, `list_slices` to show all with lease dates
   - Highlight any slices expiring within 24 hours

2. **Renew**:
   - `renew_slice(slice_name, days=N)` — extend by N days
   - Maximum renewal is typically 14 days from now
   - If the user doesn't specify days, suggest 14 (the maximum)

3. **Verify**:
   - `get_slice(slice_name)` — confirm new `lease_end`
