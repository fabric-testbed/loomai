...
- **LoomAI CLI output modernization (2026-06-14)**:
  ...
  - **Command inventory and ownership**: Audit the full CLI command matrix across `cli/loomai_cli/` and `backend/cli/loomai_cli/`. Package metadata drift has been resolved: both mirrors now declare `loomai-cli` `0.7.0` and Python `>=3.11`, and the mirrored package/test trees are kept byte-for-byte equivalent after CLI edits. The canonical edit target is documented as `cli/loomai_cli/`, with `backend/cli/loomai_cli/` mirrored for Docker/backend packaging.
