...
- **`loomai` CLI**: Broad terminal surface for FABRIC, Chameleon, federated slices, files, artifacts, monitoring, scheduling, Jupyter, AI tools, settings, and user workflows; interactive shell with `/ask`, `/model`, tab completion
...
- **Phase 4f — Final Chameleon Items (2026-03-27)**: L2 Stitch with VLAN negotiation (`POST /api/chameleon/negotiate-vlan`). Federated submit, including the legacy named submit compatibility path (`POST /api/slices/{name}/submit-composite`), with parallel FABRIC+Chameleon provisioning via asyncio.gather. Chameleon networks + floating IPs as editor topology elements. Trovi already implemented (discovered existing backend + frontend).
- **Phase 4e — Cross-Testbed Integration (2026-03-27)**: Chameleon nodes in the Federated Slice editor (FABnet v4). Server-side graph merge via `build_chameleon_slice_node_elements()`. EditorPanel gated by `chameleonEnabled` prop (Federated Slice only, not FABRIC view). ChameleonNodeForm with site/type/image/connection dropdowns. SliverComboBox Chameleon group.
...
- **LoomAI CLI output modernization (2026-06-14)**:
  - **Output policy**: Make the default output for every command human-readable and terminal-friendly. Add structured `json` output where it is useful for automation, but explicitly document rare exemptions instead of forcing JSON onto commands where it does not make sense.
  - **Command inventory and ownership**: Audit the full CLI command matrix across `cli/loomai_cli/` and `backend/cli/loomai_cli/`. Package metadata drift has been resolved: both mirrors now declare `loomai-cli` `0.7.2` and Python `>=3.11`, and the mirrored package/test trees are kept byte-for-byte equivalent after CLI edits. The canonical edit target is documented as `cli/loomai_cli/`, with `backend/cli/loomai_cli/` mirrored for Docker/backend packaging.
  - **Implementation checkpoint**: Started on `cli-update`. The shared formatter now supports callable table columns and summarizes nested list/dict cells for terminal output; `--format table|json|yaml` now works after subcommands as well as at the root; first new command coverage includes `loomai status`, `loomai schedule ...`, `loomai ai rag ...`, `loomai ai models default|set-default|test|refresh`, and AI agent/skill list/show/reset/delete commands. A second pass adds `settings get|set|test|test-all|test-provider`, safer `config` operational commands, slice state/site-resolution/availability/storage commands, network IP-hint/L3/FABNet/auto-configure commands, facility-port update, and `port-mirrors add|update|remove`. A third pass adds focused Chameleon parity (`status`, node types/detail, facility ports, availability/calendar, instance show/unaffiliated/associate-ip/execute-recipe, keypair ensure, Chameleon boot-config, slice graph/readiness/import/bastion/auto-network-setup, draft show/update-node/interface/floating-IP/graph/precreate-lease commands) plus a first-class `federated` command group for slices, members, connections, connection plans, graphs, and submit. A fourth pass adds AI tool lifecycle/status/disk checks, Aider/OpenCode web lifecycle controls, explicit AI config propagation, `tunnels list|open|close`, `jupyter start|stop|status`, notebook launch/reset/status/publish-fork, `experiments` template/readme/script/load/save commands, and `users` list/switch/delete/migrate-current. A fifth pass adds AI agent/skill create/edit, Trovi list/search/show/tags/get, monitoring node/history/infrastructure/Chameleon commands, and Chameleon OpenStack/password-project helper commands. A sixth pass adds `loomai files` for local storage, FABRIC VM files, Chameleon instance files, provisioning rules, and boot execution status/logs, and resolves package metadata drift. A seventh pass updates README, ARCHITECTURE, CONVENTIONS, and TESTING with current command coverage, output policy, streaming exceptions, and mirror ownership. An eighth pass normalizes remaining federated/artifact/boot-config mutations, converts legacy Chameleon commands to shared table/json/yaml output, adds Chameleon private-key upload, refreshes help-menu CLI docs, and expands regression tests. A ninth pass hides the legacy `composite` CLI alias from root help and switches user-facing CLI docs/tests/settings examples to `federated`. Remaining future work is limited to optional JSONL/final-summary modes for stream-oriented commands if automation demand appears.
  - **New feature command coverage**: Bring recently added LoomAI features up to CLI parity before spending time on lower-value legacy polish. Command names below are the desired user-facing shape and can be adjusted during implementation if the existing Click hierarchy suggests a cleaner fit.
  - **AI, RAG, agents, and tools**: Added `loomai ai rag status|search|rebuild`; `loomai ai models default|set-default|test|refresh`; management commands for user-custom agents and skills (`list|show|create|edit|reset|delete`); AI tool commands for install/uninstall/status/disk-space; web-tool lifecycle commands for Aider/OpenCode start/stop/status; and `loomai ai propagate-config` for explicit AI config propagation. Remaining work: explicit streaming policy for long-running AI commands.
  - **Setup, settings, and health**: Added concise `loomai status` backed by `/api/health/detailed`; expanded `config` with login/logout/token paste, auto-setup, check-update, rebuild-storage, view-status, AI tool toggles, and tool-config reset coverage; added `settings get|set|test|test-all|test-provider`; added user/profile commands for list, switch, delete, and migrate-current. Remaining work: review whether any legacy save/import setup commands need aliases.
  - **Scheduling and reservations**: Added a `schedule` group for `calendar`, `next-available`, `alternatives`, and `reservations list|create|delete`, matching the resource calendar and future reservation work already present in the WebUI/backend.
  - **FABRIC workflow gaps**: Added CLI coverage for `slices resolve-sites`, `slices check-availability`, `slices state`, `slices reconcile-projects`, `slices save-to-storage`, `slices storage-files`, and `slices open-from-storage`; added network IP hint and L3 config commands (`networks ip-hints get|set|apply`, `networks l3-config get|set`, `networks auto-configure`); added facility-port update; added `port-mirrors add|update|remove`; and exposed boot execution running/log state under `loomai files boot`.
  - **Chameleon workflow gaps**: Expanded `chameleon` with status, node-type/detail, facility-port, find-availability, schedule-calendar, OpenStack request, and password-auth project helpers. Added instance commands for `show`, `unaffiliated`, `associate-ip`, and `execute-recipe`; keypair commands for `ensure` and `upload-private-key`; Chameleon boot-config get/set/run commands; slice commands for `show|all|state|graph|auto-network-setup|import-reservation|ensure-bastion|check-readiness`; and draft commands for `show|update-node|set-node-network|set-interfaces|floating-ips|graph|precreate-leases`. Private-key upload is available for existing keys; retrieval is intentionally not exposed because the backend does not provide a safe retrieval endpoint.
  - **Federated slice commands**: Added a first-class `federated` command group matching the newer provider-slice model: `list|show|create|delete`, generic `members list|add|remove|replace-fabric`, `connections list|add|remove|set|clear`, `graph`, and `submit --wait`. Included Facility Port L2 and VLAN negotiation fields in connection commands so the CLI can exercise the recent cross-testbed work.
  - **Files, tunnels, Jupyter, and experiments**: Added `tunnels list|open|close`; added `jupyter start|stop|status` plus notebook launch/reset/status/publish-fork; added `experiments list|create|show|update|delete|readme|scripts|load|save|template|load-experiment`; and added a broader `files` group for local storage plus FABRIC VM and Chameleon instance file operations (`list|read|write|mkdir|delete|upload|download|execute`), VM reboot, provisioning rules, and boot execution status/logs.
  - **Marketplace and monitoring gaps**: Kept the existing `artifacts` commands and added Trovi marketplace coverage (`list|search|show|tags|get`). Expanded monitoring with node-level enable/disable, metrics history, infrastructure load views, and Chameleon monitoring enablement.
  - **Format option ergonomics**: Root and natural subcommand placement are supported, such as `loomai slices list --format json`. README, architecture, conventions, and testing docs now use the supported syntax and describe the output contract.
  - **Shared output contract**: Current non-streaming commands return a structured object/list for `json`/`yaml` and a clear summary/table for humans. Mutating commands return useful automation data instead of only printing prose.
  - **Human table quality**: Improve table rendering for nested dict/list fields so the default view remains readable instead of showing large inline JSON blobs. Formatter coverage now includes computed-column table cases so user-facing tables stay readable.
  - **Chameleon CLI consistency**: Chameleon commands use the shared output/error pattern for current non-streaming coverage. Legacy `json.dumps()` branches and prose-only mutation paths have been replaced with consistent table/json/yaml behavior where meaningful.
  - **Operational and streaming commands**: Treat `ssh`, `exec`, `scp`, `rsync`, `weaves logs`, `ai chat`, raw upload/download commands, and `slices export` separately. Preserve terminal/log streaming by default; add JSONL event streams or final summary JSON only where that helps automation. The intentional stream-oriented behavior is now documented in README, ARCHITECTURE, and CONVENTIONS.
  - **Docs and examples**: CLI examples now show the human default first and JSON automation second in README, architecture docs, and the WebUI help menu.
  - **Regression coverage**: The mocked command-output matrix covers natural `--format` placement, JSON parseability for representative feature groups, and the latest federated/artifact/boot-config/Chameleon cleanup paths. Streaming-command exemptions are documented.
...
- **Federated slice + base slice test expansion** — The current federated test suite (`tests/integration/test_composite.py`, legacy filename) has 28 tests covering CRUD, members, graph merge, submit, migration, and Chameleon interface rendering. Expand with:
  - **Backend API tests**:
...
- ~~**FABRIC view — Editor panel tab renaming**~~ — Done. FABRIC view shows "Slice"/"Slivers" tabs; Federated Slice view shows "Experiment"/"FABRIC"/"Chameleon" via `viewContext` prop.
- ~~**Federated Slice view — cross-testbed editor**~~ — Done. Cross-testbed topology (FABnet v4 + L2 Stitch), parallel provisioning, editor tab restructure (Experiment/FABRIC/Chameleon), unified graph.
...
- ~~**Settings — per-view enable/disable toggles**~~ — Done (2026-03-30). `views.federated_enabled` in settings.json (default `false`), with compatibility fallback for older `views.composite_enabled` settings. `GET /api/views/status` endpoint. TitleBar filters with the federated view flag. ConfigureView presents "Enable Federated Slices View" in the Chameleon section. Chameleon already gated by `chameleon.enabled`. FABRIC always visible. Only FABRIC shown on first launch.
...
- ~~**Slice editor: Chameleon nodes**~~ — Done (Phase 4e+4f). FABnet v4 + L2 Stitch with VLAN negotiation. Federated submit provisions FABRIC + Chameleon in parallel via asyncio.gather.
...
- **Federated Slice View — unified multi-testbed UI** — The Federated Slice view aggregates slices from other views into a single cross-testbed view. It shows merged topology, map, and status but does not duplicate editing functionality.
  ...
  - ~~**LoomAI branding for federated bar**~~ — Done. Replaced indigo (`#312e81` → `#6366F1`) with LoomAI brand colors (`#1c2e4a` → `#27aae1`). Updated federated bar styling, editor panel, Slices tab badges, and shared-network graph style.
...
- **Federated resource membership and cross-testbed connections** — In progress with the core FABNetv4 path implemented under federated naming while preserving composite compatibility.
  ...
  - **Terminology migration follow-up**: User-facing CLI, settings, help/docs, AI prompt text, and branding text should say "Federated Slice". Keep old `composite` API paths, hidden CLI alias, saved-data keys, CSS/test selectors, and file names as compatibility/internal surfaces until there is a deliberate migration window. A later cleanup can rename internal code symbols, tests/spec filenames, asset folders, and compatibility storage once existing saved slices and automation have a supported migration path.
...
- ~~**Federated editor panel — three tabs for editing**~~ — Done. Federated/FABRIC/Chameleon tabs support member slice pickers with checkboxes for FABRIC + Chameleon slices, status summary, inline FABRIC editing, and inline Chameleon editing. Bidirectional sync — edits in the federated editor reflect in native views and vice versa.
...
- **Experiment templates** — Save/load federated slice definitions as experiment templates. Templates capture member slice references + cross-connections + variables.
...
- ~~**Cross-testbed experiment templates**~~ — Done (Phase 9). `experiment.json` format capturing FABRIC + Chameleon resources. Save/load experiment endpoints with variable substitution. "Save as Experiment" button in the Federated Slice toolbar. Variable popup for parameterized loading. "Experiment" category in artifact system (purple badge). 28 new tests.
